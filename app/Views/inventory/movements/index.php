...
<div class="card border-0 shadow-sm"><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Número</th><th>Fecha</th><th>Tipo</th><th>Bodega</th><th>Registrado por</th><th>Despacho</th><th>Valoración</th><th></th></tr></thead><tbody>
<?php $typeLabels = ['ENTRY' => 'Entrada', 'EXIT' => 'Salida', 'ADJUSTMENT' => 'Ajuste', 'REVERSAL' => 'Reversión']; ?>
<?php foreach ($records as $record): ?><tr><td><span class="font-monospace"><?= esc($record['movement_number']) ?></span><?php if ($record['document_number']): ?><div class="small text-body-secondary"><?= esc($record['document_number']) ?></div><?php endif ?></td><td><?= esc($record['created_at']) ?></td><td><?= esc($typeLabels[$record['type']] ?? $record['type']) ?><?php if ((int) $record['is_reversed'] === 1): ?> <span class="badge text-bg-warning">Revertido</span><?php endif ?></td><td><?= esc($record['warehouse_name']) ?></td><td><?= esc($record['created_by_name']) ?></td><td><?php if ($record['type'] === 'EXIT'): ?><?= (int) $record['has_pending_dispatch'] === 1 ? '<span class="badge text-bg-warning">Parcial/Pendiente</span>' : '<span class="badge text-bg-success">Completo</span>' ?><?php else: ?>—<?php endif ?></td><td><?= (int) $record['current_pending_valuation'] === 1 ? '<span class="badge text-bg-warning">Pendiente</span>' : '<span class="badge text-bg-success">Completa</span>' ?></td><td class="text-end"><a class="btn btn-sm btn-outline-primary" href="<?= url_to('inventory-movement-show', $record['id']) ?>">Ver</a></td></tr><?php endforeach ?>
<?php if ($records === []): ?><tr><td colspan="8" class="text-center text-body-secondary py-4">No hay movimientos para los filtros seleccionados.</td></tr><?php endif ?>
