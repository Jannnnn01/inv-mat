...
<?php foreach ($records as $record): ?><tr><td class="font-monospace"><?= esc($record['movement_number']) ?></td><td><?= esc($record['created_at']) ?></td><td><?= esc($typeLabels[$record['type']] ?? $record['type']) ?><?php if ((int) $record['is_reversed'] === 1): ?> <span class="badge text-bg-warning">Revertido</span><?php endif ?></td><td><?= esc($record['warehouse_name']) ?></td><td><?= esc($record['created_by_name']) ?></td><td><?= $record['has_pending_valuation'] ? '<span class="badge text-bg-warning">Pendiente</span>' : '<span class="badge text-bg-success">Completa</span>' ?></td><td class="text-end"><a class="btn btn-sm btn-outline-primary" href="<?= url_to('inventory-movement-show', $record['id']) ?>">Ver</a></td></tr><?php endforeach ?>
